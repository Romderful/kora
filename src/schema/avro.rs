...
pub fn check_compatibility(
    ...
    let existing = Schema::parse_str(existing_schema)
        .map_err(|e| KoraError::InvalidSchema(e.to_string()))?;
    ...
    let result = match direction {
        ...
        CompatDirection::None => return Ok(CompatibilityResult {
            is_compatible: true,
            messages: Vec::new(),
        }),
    };
...
fn collect_error_messages(err: &CompatibilityError) -> Vec<String> {
    ...
    if let Some(inner) = std::error::Error::source(err)
        .and_then(|s| s.downcast_ref::<CompatibilityError>())
    {
